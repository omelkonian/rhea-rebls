...
\acmConference[REBLS'18]{International Workshop on Reactive and Event-Based Languages \& Systems}{November 04, 2018}{Boston, MA, USA}
\acmYear{2018}
\acmISBN{}
\acmDOI{}
\startPage{1}
...
graph can be evaluated by a different \texttt{EvaluationStrategy} which could
...
Source nodes are constructed using built-in functions of the \texttt{Stream} type.
...
code.
%
\codefig{hamming}{\lscalaxs{code/hamming.scala}}{Hamming numbers}
%
...
namely \texttt{multiply} (line 7), which just multiplies the stream with a constant, and
\texttt{mergeSort} (line 8), which produces an ordered stream given two ordered
...
\texttt{faceDetect} (line 6), \texttt{embedLaser} (line 7) and
\texttt{toGray} (line 14)) are not shown for brevity's sake. It is worth noting
...
\textit{ReactiveBeacons}\footnote{\url{http://github.com/pwittchen/ReactiveBeacons}}
...
This is exactly what \textit{Ziria} accomplishes in the domain of wireless
systems programming~\cite{ziria}. The driving force
