...
Robotics and Internet of Things (IoT) are two domain where the reactivity of
applications is imperative. A typical application in these domains needs to timely
and continuously respond to time-varying external stimuli. As a result the
domain programmer needs to implement tedious and often error-prone behaviours
...
proposal with the main purpose to alleviate from the programmer the need to
deal with implementation chores. To achieve that a typical FRP program uses
higher-order functional operators to define a dataflow network of processing nodes.
...
and several other proposals exist for Robotics applications~\cite{}
...
data structures and common graph-theoretic algorithms\site{http://jgrapht.org/}.
...
the \textit{Hazelcast}\site{http://hazelcast.org/} library to discover and
...
\optimizationh{maptake}{Take/skip/distinct before map}
\optimizationh{mapfilter}{Filter before map}
...
\optimizationh{mergemap}{Merge maps}
\optimizationh{combmapfilter}{Combine map with filter}
\optimizationh{combfilterexists}{Combine filter with exists}
\optimizationh{combmapexists}{Combine map with exists}
\optimization{combmapzip}{Combine map with zip}
\optimizationh{combzipmap}{Combine zip with map}
